<script setup lang="ts">
  ...
  const navigation = [
    ...
    { name: 'Account', href: '/account', icon: "codicon:account"},    
    { name: 'Market', href: '/market', icon: "icon-park-outline:weixin-market"},    
  ]
  const teams = [
  ...
  const userNavigation = [
    { id: 1, name: 'Profile', href: '/account',icon: "iconamoon:profile" },
    { id: 2, name: 'Sign out', href: '/login',icon: "gridicons:sign-out" },
  ]
              ...
              <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
                ...
                <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-purple-600 px-6 pb-4">
                  <div class="flex h-16 shrink-0 items-center">
                    <ApplicationLogo class="overflow-hidden rounded-lg w-[45px]" />
                  </div>
                  <nav class="flex flex-1 flex-col">
                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                      <li>
                        <ul role="list" class="-mx-2 space-y-1">
                          <li v-for="item in navigation" :key="item.name">
                            <NuxtLink :to="item.href" :class="[isCurrentPage(item.href) ? 'bg-purple-700 text-white' : 'text-purple-200 hover:bg-purple-700 hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']">
                      ...
                      <li>
                        ...
                        <ul role="list" class="-mx-2 mt-2 space-y-1">
                          <li v-for="team in teams" :key="team.name">
                            <NuxtLink :to="team.href" :class="[isCurrentPage(team.href) ? 'bg-purple-700 text-white' : 'text-purple-200 hover:bg-purple-700 hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']">
                      ...
                      <li class="mt-auto">
                        <NuxtLink :to="settingsHref" :class="[isCurrentPage(settingsHref) ? 'bg-purple-700 text-white' : 'text-purple-200 hover:bg-purple-700 hover:text-white', 'group -mx-2 flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']">
            ...
            <div class="flex items-center gap-x-4 lg:gap-x-6">
              ...
              <Menu as="div" class="relative">
                <MenuButton class="-m-1.5 flex items-center p-1.5">
                  ...
                  <img class="h-8 w-8 rounded-full bg-gray-50" src="https://media.hozehonari.ir/d/old/wp-content/uploads/2020/12/48484.jpg" alt="" />
                  <span class="hidden lg:flex lg:items-center ml-3">
                    <div class="hidden lg:grid">
                      ...
                      <div class="row">
                        <span v-if="user?.email" class="font-medium text-xs text-gray-500">
                          {{ user.email }}
                        </span>
                      </div>
                    </div>
                    ...
                    <MenuItem 
                      ...
                      <template v-if="item.id === 2">
                        <a 
                          href="#" 
                          @click="logout" 
                          :class="[active ? 'bg-gray-50' : '', 'flex justify-between items-center px-3 py-1 text-sm leading-6 text-gray-900']">
                          {{ item.name }}
